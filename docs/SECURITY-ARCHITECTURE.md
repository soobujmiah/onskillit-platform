...
- **Recovery/backups:** assisted mobile-only reset requires a trained restricted support procedure and privileged second approval before live use; no OTP is mandatory V1. Encrypt backups, separate credentials, rehearse restore, redact logs and protect GitHub PR workflows from production secrets.

[ADR 0007](decisions/0007-browser-auth-and-rbac.md) accepts session/RBAC and [ADR 0008](decisions/0008-payment-adapter-contract.md) accepts commerce trust boundaries. Security architecture is **PASS**; detailed proofing, retention and provider-specific controls are release procedures and later phase gates. No deployed control has been tested.


## Accepted minimum controls

Use Argon2id at no less than OWASP's current minimum, unique per-password salts and parameter migration; never store recoverable passwords. Session token hashes, rotation/revocation, Secure/HttpOnly/SameSite cookies, CSRF token and origin checks are mandatory. Password reset tokens are single-use, short-lived and hashed; staff-assisted phone reset requires trained proofing and privileged dual control. Scope every read and write by user/client account and role assignment, including search, media and exports. Enforce validation at API boundary and domain invariants in PostgreSQL; parameterize all queries and encode/sanitize authored HTML. No raw executable CMS blocks. SSRF-sensitive outbound HTTP uses allowlisted provider endpoints, DNS/IP controls, timeouts and egress policy. Uploads are size/type/magic-byte checked, scanned according to policy, processed in isolation and private until approved. Payment callbacks authenticate per provider and require independent query plus idempotent amount/currency/reference validation. Secrets are only in protected environment configuration; public PR CI has no production secrets. Backups are encrypted offsite with least-privilege restore access. Security review and negative GitHub tests are required from the first owning implementation phase; PHASE-12 hardens and independently verifies, rather than inventing these controls.
